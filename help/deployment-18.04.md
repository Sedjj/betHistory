...
Для аутентификации во время соединения, используя пользователя vivek и пароль для базы данных администратора:
 ```bash
mongo --port 27017 -u adminRate -p 7bdeW7XkuzI7uI3pa5k6e7984W7XkuzI7VAFnIuFAir774645c2b543d --authenticationDatabase admin
```


Добавьте дополнительного пользователя в вашу БД. Сначала создайте новую базу данных под названием «nixcraft»:
 ```bash
use rateBotNest
```                  

Создайте пользователя с именем «rate» с паролем с именем «Passowrd» для rate db:
```bash   
db.createUser(
  {
   user: "rate",

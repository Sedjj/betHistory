...
/*async function result(statistics, beforeDate, currentDate, setStatistic, numericalDesignation) {
	try {
		const currentData = await getResultList(searchHelper.replaceUrl(urlAll, currentDate));
		const beforeData = await getResultList(searchHelper.replaceUrl(urlAll, beforeDate));
